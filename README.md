...
4|Account|1. "Add" label in the Accounts menu appears confusing. Tapping on "Add" is not functional. New account can be added only by tapping "+". 2. Account with same name, currency and icon can be added (duplicate accounts can appear confusing when managing transactions) 3. Only the initial account balance of the cash account from default is included in the Balance, Transaction list and Chart. The initial account balance for all other accounts (payment card from default and all other new accounts added)are not included even though the switcher "Included in the balance" is turned on|Usability (1, 2), Reliability (3)|1. Make the "Add" label functional as the "+" symbol or remove the "Add" label from the menu 2. Add quick user guide on unique account creation i.e. by name and currency, icon and currency, etc. Impose validation on duplicate accounts 3. Review the criteria to include the account balance in the Balance, Chart and Transaction list, and rectify the solutions where appropriate.
5|Income|1. Income added to accounts (other than cash account from defaults) is not updated to the Balance, Transaction list and Chart even though the switcher "Included in the balance" is turned on 2. When an income record is edited, there is no option to cancel Edit. User could have made some mistakes in the edit and wants to undo changes, but option to cancel changes is not available. User only has the option to undo changes after the changes are saved (Done)|Reliability (1), Usability (2)|1. Review the criteria to include the Income transaction in the Balance, Chart and Transaction list, and rectify the solutions where appropriate. 2. Add a cancel option for Edit.
6|Expense|1. Expense added to accounts (other than cash account from defaults) is not updated to the Balance, Transaction list and Chart even though the switcher "Included in the balance" is turned on 2. When an expense record is edited, there is no option to cancel Edit. User could have made some mistakes in the edit and wants to undo changes, but option to cancel changes is not available. User only has the option to undo changes after the changes are saved (Done).|Reliability (1), Usability (2)|1. Review the criteria to include the Expense transaction in the Balance, Chart and Transaction list, and rectify the solutions where appropriate. 2. Add a cancel option for Edit.
...
8|Chart|1. Only expense transactions of the cash account from defaults are reflected in the Chart. Expense transactions of all other accounts are not reflected in the Chart. 2. Total spending distribution exceeeds 100%|Reliability (1, 2)|1. Review the criteria to include the expense transactions in the Chart, and rectify the solutions where appropriate. 2. Rectify the rounding precision of the spending distribution
9|Balance|1. Only account balance and transactions of the cash account from defaults are reflected in the Balance. Account balances and transactions of all other accounts are not reflected in the Balance. 2. Monthly budget amount is not reflected in the Chart when Budget mode is turned on. The features on Budget mode cannot be fully tested (feature is not usuable from end user's perspective)|Reliability (1), Usuability (2)|1. Review the criteria to include the account balance, income and expense transactions in the Balance, and rectify the solutions where appropriate. 2. Quick user guide as to how budget mode can be used i.e. monthly budget amount vs income and carry over, and review solutions for variations of balance settings
